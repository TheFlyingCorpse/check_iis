...
namespace MonitoringPluginsForWindows
{
    public class check_iis
    {
        ...
        private static bool InventoryOutputI2Conf(bool do_inventory_apppools, bool do_inventory_sites, bool do_skip_empty_vars, bool bVerboseInventory)
        {
            ...
            foreach (var IISInventoryLocal in listIISInventory)
            {
        ...
        private static Array InventorySiteAppVirtualDirectories(VirtualDirectoryCollection Application, bool bVerboseInventory)
        {
            ...
            foreach (var VirtualDirectory in Application)
            {
        ...
        public static int CheckAllSites(int returncode, bool do_perfcounter_sites, bool do_singluar_check, string expected_state)
        {
            ...
            foreach (var site in listOverSites.Values)
            {
                ...
                else if (do_singluar_check == true && bExpectedStateSet == true)
                {
                    ...
                    {
                        returncode = ReturnCodeMagic(returncode, (int)ServiceState.ServiceWarning);
                    }
                    else
                    {
                        returncode = ReturnCodeMagic(returncode, (int)ServiceState.ServiceCritical);
                    }
        ...
        public static int CheckAllAppPools(int returncode, bool do_perfcounter_apppools, bool do_singluar_check, string expected_state)
        {
            ...
            foreach (var apppool in listOverAppPools.Values)
            {
                ...
                {
                    output = "AppPool '" + sAppPoolName + "' not set to AutoStart and apppool state is '" + apppool.State.ToString() + "'";
                }
                // OK
                ...
                {
                    if (bAppPoolShouldBeWarned == true)
                    {
                        returncode = ReturnCodeMagic(returncode, (int)ServiceState.ServiceWarning);
                    }
                    else
